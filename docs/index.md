...
## 🔤 **Phase 2.7: 스마트 자막 줄바꿈 시스템 계획 (2025.05.27)**

### 📋 **개발 계획: 단어 단위 줄바꿈 (A방식)**

#### 🎯 **해결할 문제**
- **현재 상황**: '위하여' → '위' + '하여' 처럼 단어 중간에서 분할
- **목표**: 단어의 완전성 보장, 자연스러운 줄바꿈

#### 🛠️ **기술적 접근 (A방식: 단어 단위 분할)**
```python
# 구현 계획
1. SRT 생성 단계에서 텍스트 전처리
2. 공백 기준 어절 분리
3. 줄 길이 제한 내에서 단어 완전성 유지
4. 초과시 다음 줄로 전체 단어 이동
```
...
#### 📝 **구현 단계**
1. **1단계**: `generate_srt` 함수 개선
2. **2단계**: 단어 단위 줄바꿈 로직 추가  
3. **3단계**: 해상도별 최적 길이 설정
4. **4단계**: 테스트 및 검증
